---
name: code-review
description: "Review pull requests and diffs in investmate — the Go ETF dividend and yield analyser — against the rios0rios0/guide standards: the domain/infrastructure split, one repository per data source, Logrus structured logging, and BDD tests. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `investmate`

`investmate` scrapes and analyses ETF data — dividend cash amounts, average closing prices, and yields over a configurable number of years — and renders the result as a table. Its correctness is arithmetic: a wrong divisor or a mis-parsed currency silently produces plausible, wrong numbers.
...
## What matters most in `investmate`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

```
cmd/main.go                                    orchestrates fetching and rendering
internal/domain/entities/etf.go                ETF struct and every calculation
internal/domain/repositories/                  ports: DividendsRepository, PricesRepository
internal/infrastructure/repositories/
  nasdaq/                                      NASDAQ REST adapters (dividends, prices)
  statusinvest/, historyorg/                   crawler adapters (currently unused)
```

- **Every calculation lives on the `ETF` entity.** A yield, average, or formatting rule computed in `cmd/main.go` or inside an adapter is a layering violation and hides itself from the entity's tests.
- **A new data source is a new package under `internal/infrastructure/repositories/`** implementing the existing port. It must not require a change to the domain.
- **Percentages and money need explicit precision.** Named constants such as `YearsToFetch` and `PercentageMultiplier` exist so the arithmetic is readable — a new bare literal in a formula is a finding.
- **Live-API tests are a liability.** Some tests reach the network; a new one that does should either be gated or replaced with a fixture, and it must never depend on today's market data to pass.
- Errors are wrapped with `fmt.Errorf("…: %w", err)`; logging uses Logrus with structured fields (`logger.WithError(err).Errorf(...)`).
- Package names stay lowercase single words (`nasdaq`, `entities`); exported constants are PascalCase, unexported camelCase.
...
- `Test<Package>_<Function>` with `"should … when …"` subtests, `t.Parallel()` at both levels.
- New arithmetic needs a test with a hand-checked expected value — not one derived by running the code.
...
- The unused `statusinvest` and `historyorg` adapters existing — they are kept deliberately as alternative sources.
